...
class Evaluator:
    ...
    def _evaluate_constraints(
        ...
        for constraint in constraints:
            try:
                result = constraint.fitness(individual) # call in fitness.py
                fitness += result.fitness()             # exec sum
    ...
    def evaluate_individual(
        ...
        # ---------- ABLATION: non-linear transformation ----------
        if self._fitness_alpha != 1.0:
            fitness = fitness ** self._fitness_alpha
        # ---------------------------------------------------------
